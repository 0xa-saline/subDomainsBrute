...
from fast_sub import get_subd
from ip_translate import interpret,trs
import sys,ast
import re
import dns.resolver
import time
...
from lib.database import execute,query
...
class SubNameBrute:
    ...
    def _get_untile(self):
        self._print_msg('[+] Load search subs ...')
        normal = set()
        try:
            results = get_subd(self.target)
        except:
            results = []
        for res in results:
            normal.add(res.replace('.'+self.target,''))
    ...
    def _load_sub_names(self):
        ...
        normal_lines = []
        normal_lines.extend(self._get_untile())
        self._print_msg('[+] Load search subs finish ...')
        wildcard_lines = []
        wildcard_list = []
        regex_list = []
        lines = set()
        ...
        normal_lines = list(set(normal_lines))
            ...
            try:
                ...
                if answers:
                    ...
                    try:
                        ...
                        ipclist = ast.literal_eval(ips)

                    msg = cur_sub_domain.ljust(30) + ips
                    self._print_msg(msg, _found_msg=True)
                    self._print_msg()
                    result = {"domain":cur_sub_domain,"ip":ipclist}
                    self.sub_domain.append(result)
                    try:
    ...
    def run(self):
        ...
        try:
            ...
            query = str(self.target),str(self.taskid),json.dumps(list(totail)),str(datetime.datetime.now())
...
if __name__ == '__main__':
    ...
    parser.add_option('-t', '--threads', dest='threads', default=50, type=int,
                      help='Num of scan threads, 50 by default')
    parser.add_option('-o', '--taskid', dest='taskid', default=None,
